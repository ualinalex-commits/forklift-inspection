...
// Header field stamp positions (page 0)
const STAMP_MACHINE_REF = { x: 92, y: 757 };
const STAMP_SERIAL      = { x: 92, y: 739 };
const STAMP_WEEK_COMM   = { x: 395, y: 701 };
const STAMP_SITE        = { x: 92, y: 720 };
...
// Extracted via pdfjs-dist text extraction from the actual template.
// "Additional comments" label at y=748.7, diagram note at y=535.6,
// "Operator Name" row at y=362.9, "Supervisor Name" row at y=329.4.
const P3 = {
  COMMENTS_X:    27,
  COMMENTS_Y:   728,   // just below the "Additional comments" header (label at 748.7)
  COMMENTS_MAX_Y: 548, // stop before "Use diagram below..." text at 535.6
  COMMENTS_MAX_W: 540, // pts wide within page margins

  DIAGRAM_X:   27,
  DIAGRAM_Y:   367,   // bottom edge of diagram area (operator row at 362.9)
  DIAGRAM_MAX_W: 542, // pts wide (page margins 27 each side)
  DIAGRAM_MAX_H: 140, // pts tall (up to the diagram note at ~520)

  OP_NAME_X:   120,
  OP_NAME_Y:   353,
  OP_SIGN_X:   300,
  OP_SIGN_Y:   333,   // bottom edge of signature image
  OP_SIGN_MAX_W: 140,
  OP_SIGN_MAX_H: 28,
  OP_DATE_X:   465,
  OP_DATE_Y:   353,
...
// Stamp page 3 (index 2) with additional comments, annotated diagram,
// operator name, signature image, and inspection date.
async function stampPage3(pdfDoc, page, data, reg) {
  ...
  // Annotated diagram image (PNG from forklift-photos bucket)
  if (data.diagram_bytes) {
    try {
      const img  = await pdfDoc.embedPng(data.diagram_bytes);
      const dims = img.scaleToFit(P3.DIAGRAM_MAX_W, P3.DIAGRAM_MAX_H);
      // Centre horizontally in the diagram area
      const imgX = P3.DIAGRAM_X + (P3.DIAGRAM_MAX_W - dims.width) / 2;
      page.drawImage(img, { x: imgX, y: P3.DIAGRAM_Y, width: dims.width, height: dims.height });
    } catch (e) {
