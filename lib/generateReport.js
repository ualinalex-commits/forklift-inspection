...
async function generateReport(forkliftId, weekCommencing) {
  ...
  // Pick the best entry for page 3: prefer one with a supervisor sign-off already
  // recorded, then the most recent with additional_comments or diagram_annotation_url,
  // falling back to the most recent entry overall.
  const page3Entry =
    (entries || []).find(e => e.supervisor_name) ||
  ...
  if (page3Entry) {
    ...
    const [diagBytes, sigBytes, supervisorSigBytes] = await Promise.all([
      ...
      fetchImage(page3Entry.supervisor_signature_url),
    ]);
    page3Data = {
      ...
      supervisor_name:       page3Entry.supervisor_name,
      supervisor_sig_bytes:  supervisorSigBytes,
      supervisor_sign_date:  page3Entry.supervisor_sign_date,
    };
  }
