...
const FUNC_ROWS = {
  ...
  31: { page: 1, y: 437.9 }, // Accessories (Slings, bin handlers, chains etc) — midpoint of label lines 443.9/431.9
};
...
// Left column: "Serial Number" label ends at ~x=99; "Site" label ends at ~x=77. Values start at x=90.
// Right column: "Thorough/Exam/Expiry Date" and "Week/Commencing/Date" labels end around x=370. Values start at x=375.
const STAMP_SERIAL         = { x:  90, y: 739 }; // serial number value — right of "Serial Number" label
const STAMP_THOROUGH_EXAM  = { x: 375, y: 739 }; // thorough exam expiry date — right of right-column label
const STAMP_SITE           = { x:  90, y: 701 }; // site name — right of "Site" label
const STAMP_WEEK_COMM      = { x: 375, y: 701 }; // week commencing date — right of right-column label
...
async function buildPDF({ forklift, sheet, summaryByItem, operatorsByDay, defectsByDay, mediaByDay, page3Data }) {
  ...
  // Build defect comment lookup: defectLookup[day][item_number] = defect_details
  const defectLookup = {};
  for (const [day, defects] of Object.entries(defectsByDay || {})) {
    defectLookup[day] = {};
    for (const d of defects) {
      defectLookup[day][d.item_number] = (d.defect_details || "").slice(0, 20);
    }
  }
...
const P3 = {
  ...
  OP_NAME_X:   104, // "Operator Name" label ends at x≈99; value starts after gap
  OP_NAME_Y:   140.8,
