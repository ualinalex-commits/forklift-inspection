...
// ─── Append A4-portrait daily summary pages (one page per day with a photo) ──
async function appendDailySummaryPages(pdfDoc, sheet, operatorsByDay, defectsByDay, mediaByDay, reg, bold) {
  const PW = 595.28;
  const PH = 841.89;

  const daysWithPhotos = DAY_NAMES.filter(d => mediaByDay[d]?.photo_bytes);
  ...
  for (const day of daysWithPhotos) {
    ...
    try {
      const img  = await (media.photo_url?.match(/\.png$/i)
        ? pdfDoc.embedPng(media.photo_bytes)
        : pdfDoc.embedJpg(media.photo_bytes));
      const dims = img.scaleToFit(PW - 60, PH - 60);
      page.drawImage(img, {
        x: (PW - dims.width)  / 2,
        y: (PH - dims.height) / 2,
        width:  dims.width,
...
// Extracted from PL054-OP-V3 template via debugPage3Coords.mjs (node debugPage3Coords.mjs):
//   "Operator Name"  x=27.25  y=374.63  w=71.71  rightEdge=98.96
//   "Sign"           x=297.13 y=374.63  w=19.12  rightEdge=316.25
//   "Date"           x=460.45 y=374.63  w=21.76  rightEdge=482.21
//   "Supervisor Name" y=340.85 — left blank
//
// Derived positions:
//   OP_NAME_X  = "Operator Name" rightEdge + 5pt   = 98.96  + 5  = 103.96
//   Sign cell value space: "Sign" rightEdge (316.25) → "Date" x (460.45), width=144.2pt
//     → signature centred dynamically in stampPage3 using OP_SIGN_CELL_LEFT/RIGHT
//   OP_DATE_X  = "Date" rightEdge + 8pt              = 482.21 + 8  = 490.21
const P3 = {
  COMMENTS_X:     27,
  ...
  DIAGRAM_X:      27,
  DIAGRAM_Y:     388,  // bottom edge (operator row at y=374.63, +13.4pt buffer)
  DIAGRAM_MAX_W:  540,
  DIAGRAM_MAX_H:  256, // top = 388+256=644, just below "(this may not...)" at y=654.2

  // Operator sign-off row (y=374.63, extracted via debugPage3Coords.mjs)
  OP_NAME_X:          103.96, // "Operator Name" rightEdge=98.96 + 5pt
  OP_NAME_Y:          374.63,
  // Sign cell value space: from "Sign" rightEdge to "Date" x-start
  OP_SIGN_CELL_LEFT:  316.25, // "Sign" rightEdge=316.25
  OP_SIGN_CELL_RIGHT: 460.45, // "Date" x=460.45
  OP_SIGN_Y:          356.0,  // image bottom (extends from 356→374, within row)
  OP_SIGN_MAX_W:       60.0,  // signature scaled to fit 60×18pt
  OP_SIGN_MAX_H:       18.0,
  OP_DATE_X:          490.21, // "Date" rightEdge=482.21 + 8pt
  OP_DATE_Y:          374.63,
};
...
async function stampPage3(pdfDoc, page, data, reg) {
  ...
  console.log("[page3] P3 coord constants:");
  console.log(`  OP_NAME_X=${P3.OP_NAME_X}  OP_NAME_Y=${P3.OP_NAME_Y}`);
  console.log(`  OP_SIGN_CELL_LEFT=${P3.OP_SIGN_CELL_LEFT}  OP_SIGN_CELL_RIGHT=${P3.OP_SIGN_CELL_RIGHT}  OP_SIGN_Y=${P3.OP_SIGN_Y}`);
  console.log(`  OP_DATE_X=${P3.OP_DATE_X}  OP_DATE_Y=${P3.OP_DATE_Y}`);
  ...
  // Operator signature image — centred horizontally in the Sign cell value space
  if (data.sig_bytes) {
    try {
      const sig  = await pdfDoc.embedPng(data.sig_bytes);
      const dims = sig.scaleToFit(P3.OP_SIGN_MAX_W, P3.OP_SIGN_MAX_H);
      const cellW = P3.OP_SIGN_CELL_RIGHT - P3.OP_SIGN_CELL_LEFT; // 460.45-316.25 = 144.2
      const sigX  = P3.OP_SIGN_CELL_LEFT + (cellW - dims.width) / 2;
      console.log(`[page3] sig dims=${dims.width.toFixed(1)}×${dims.height.toFixed(1)}  sigX=${sigX.toFixed(2)}  sigY=${P3.OP_SIGN_Y}`);
      page.drawImage(sig, { x: sigX, y: P3.OP_SIGN_Y, width: dims.width, height: dims.height });
    } catch (e) {
