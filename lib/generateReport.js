...
// Left column:  "Serial Number" ends at x≈85.3  → value at x=90, both rows y=739 / y=701.7
// Right column: "Examination"   ends at x≈369.7 → value at x=375, both rows y=739 / y=701.7
const STAMP_SERIAL         = { x:  90, y: 739   }; // serial number — right of "Serial Number" label
const STAMP_THOROUGH_EXAM  = { x: 375, y: 739   }; // thorough exam expiry — right of "Examination" label
const STAMP_SITE           = { x:  90, y: 701.7 }; // site name — right of "Size" label (y=701.7)
const STAMP_WEEK_COMM      = { x: 375, y: 701.7 }; // week commencing — right of "Commencing" label (y=701.7)
...
async function buildPDF({ forklift, sheet, summaryByItem, operatorsByDay, defectsByDay, mediaByDay, page3Data }) {
  ...
  for (const [day, defects] of Object.entries(defectsByDay || {})) {
    ...
    for (const d of defects) {
      const raw = d.defect_details || "";
      defectLookup[day][d.item_number] = raw.length > 40 ? raw.slice(0, 37) + "..." : raw;
    }
  }
  ...
  for (let item = 1; item <= 20; item++) {
    ...
    for (const day of DAY_NAMES) {
      ...
      if (result === "fail" && !commentStamped) {
        ...
        if (comment) {
          targetPage.drawText(comment, { x: COMMENTS_COL_X[row.page] ?? 503, y: row.y, size: 6, font: reg, color: DARK });
          commentStamped = true;
        }
      }
    }
  ...
  for (let item = 21; item <= 31; item++) {
    ...
    for (const day of DAY_NAMES) {
      ...
      if (result === "fail" && !commentStamped) {
        const comment = defectLookup[day]?.[item] || "";
        if (comment) {
          targetPage.drawText(comment, { x: COMMENTS_COL_X[row.page] ?? 506.5, y: row.y, size: 6, font: reg, color: DARK });
          commentStamped = true;
        }
      }
    }
...
// Re-extracted from updated PL054-OP-V3 template (June 2026) via pdfjs-dist:
//   "Additional comments" label  y=767.7
//   "Use diagram below..."        y=667.7
//   "(this may not exactly...)"   y=654.2
//   "Operator Name"  x=27.3  y=374.6  w=71.7  (ends x=99.0)
//   "Sign"           x=297.1 y=374.6  w=19.1  (ends x=316.2)
//   "Date"           x=460.5 y=374.6  w=21.8  (ends x=482.3)
//   "Supervisor Name" y=340.9 — left blank
const P3 = {
  COMMENTS_X:     27,
  ...
  DIAGRAM_X:      27,
  DIAGRAM_Y:     388,  // bottom edge (operator row at y=374.6, +13.4pt buffer)
  DIAGRAM_MAX_W:  540,
  DIAGRAM_MAX_H:  256, // top = 388+256=644, just below "(this may not...)" at y=654.2

  OP_NAME_X:   104,    // after "Operator Name" label ending at x=99.0
  OP_NAME_Y:   374.6,  // "Operator Name" label baseline
  OP_SIGN_X:   320,    // just right of "Sign" label ending at x=316.2
  OP_SIGN_Y:   353,    // bottom edge of signature image (top aligns with row baseline ~374.6)
  OP_SIGN_MAX_W: 130,
  OP_SIGN_MAX_H:  20,
  OP_DATE_X:   487,    // just right of "Date" label ending at x=482.3
  OP_DATE_Y:   374.6,  // same row as Operator Name and Sign
};
